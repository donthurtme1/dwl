...
void createpointer(struct wlr_pointer *pointer) {
	...
	if (wlr_input_device_is_libinput(&pointer->base)
        ...
        //if (libinput_device_config_calibration_get_matrix(device, calibration_matrix))
            //libinput_device_config_calibration_set_matrix(device, calibration_matrix);
	}
...
void tile(Monitor *m) {
	...
	if (n > m->nmaster)
		ms = m->nmaster ? ROUND(width * m->mfact) : 0;
	else
		ms = width;
	i = my = ty = 0;
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			resize(c, (struct wlr_box){
					...
					.y = y + my + wsep,
					.width = ms - (2 * wsep),
					.height = (height - my) / (MIN(n, m->nmaster) - i) - (2 * wsep) }, 0);
			my += c->geom.height + (2 * wsep);
		} else {
			resize(c, (struct wlr_box){
					.x = x + ms + wsep,
					.y = y + ty + wsep,
					.width = width - ms - (2 * wsep),
					.height = (height - ty) / (n - i) - (2 * wsep) }, 0);
			ty += c->geom.height + (2 * wsep);
		}
		i++;
	}
}
...
void iter_xdg_scene_buffers_shadow(struct wlr_scene_buffer *buffer, int sx, int sy, void *user_data) {
	Client *c = user_data;
	struct wlr_scene_surface * scene_surface = wlr_scene_surface_try_from_buffer(buffer);
	struct wlr_xdg_surface *xdg_surface;
